...
Imports in the @Component decorator - To use a component, directive, or pipe, you must add it to the imports array in the @Component decorator.
...
6. The `ngOnDestroy` method is a lifecycle hook in Angular that is called just before a component or directive is destroyed and removed from the DOM. It is used to perform cleanup tasks and unsubscribe from subscriptions.
...
It is not a specific feature provided by Angular.Instead, it is just like a normal component,which gets created at runtime.We create or render dynamic component by writing same code.

We can render dynamic component in two ways:-
