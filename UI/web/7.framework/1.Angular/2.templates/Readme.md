...
```html
<button class="btn btn-primarty" [class.active]="isActive">Save</button>
```
...
Angular pipes takes data as an input and it formats or transforms that data before displaying it on the template.
...
**Built-in Pipes**:- Angular includes a set of built-in pipes in the @angular/common package:
...
`Using pipes` - Angular's pipe operator uses the vertical bar character (|), within a template expression. The pipe operator is a binary operator– the left-hand operand is the value passed to the transformation function, and the right side operand is the name of the pipe and any additional arguments
...
`Combining multiple pipes in the same expression`:- You can apply multiple transformations to a value by using multiple pipe operators. Angular runs the pipes from left to right.
...
`Change detection with pipes`:- By default, all pipes are considered pure, which means that it only executes when a primitive input value (such as a String, Number, Boolean, or Symbol) or a object reference (such as Array, Object, Function, or Date) is changed. Pure pipes offer a performance advantage because Angular can avoid calling the transformation function if the passed value has not changed.
...
**Custom Pipes**:- You can define a custom pipe by implementing a TypeScript class with the @Pipe decorator. A pipe must have two things:

1. A name, specified in the pipe decorator
2. A method named transform that performs the value transformation.
...
Using the @Pipe decorator - When creating a custom pipe, import Pipe from the @angular/core package and use it as a decorator for the TypeScript class.
...
Implement the PipeTransform interface - In addition to the @Pipe decorator, custom pipes should always implement the PipeTransform interface from @angular/core.
Implementing this interface ensures that your pipe class has the correct structure.

Adding parameters to a custom pipe - You can add parameters to your transformation by adding additional parameters to the transform method:

```ts
transform(value: string, format: string): string {}
```

Detecting change within arrays or objects - When you want a pipe to detect changes within arrays or objects, it must be marked as an impure function by passing the pure flag with a value of false.
...
```html
<ul>
   <li *ngFor="let course of courses; index as i">{{ i }} - {{ course }}</li>
</ul>
```
...
<main [ngSwitch]="activeTab">
  <h2 *ngSwitchCase="'personal'">This is personal info</h2>
  <h2 *ngSwitchCase="'security'">This is security info</h2>
  <h2 *ngSwitchCase="'booking'">This is booking info</h2>
  <h2 *ngSwitchDefault>No selected</h2>
</main>
```
...
<span [ngClass]="{'icon-star': isSelected, 'icon-star-empty': !isSelected}"></span>
```
...
Angular templates support control flow blocks that let you conditionally show, hide, and repeat elements.Angular v17 introduced new declarative syntax feature like `@if`,`@for` & `@switch` to replace old structural directives like *ngIf,*ngFor & *ngSwitch.

1. It is cleaner and more readable:- @if and other declarative syntax mirrors Javascript control flow and we do not need to use <ng-templates> blocks or <ng-container> wrapper.This reduces template noise & makes nested branches easier to follow.
2. No imports or directives to manage:- @if and other declarative syntax are built into template engine.So, you don't need to import anything to use it.It cuts boilerplate coe & avoids missing import errors.
3. Performance oriented compilation
4. Future ready(Zoneless & signals) - Aligns with Angular's move towards zoneless apps and signal based reactivity.
...
`Providing a fallback for @for blocks with the @empty block`:- You can optionally include an @empty section immediately after the @for block content. 
