...
The image is a collection of files and metadata.That metadata includes the specific program to execute and other relevant configuration details.
...
1. FROM - Create a new build stage from a base imageuse for base images or to pull an image (can be used multiple times).
2. RUN - Execute build commands(for installing software or running Linux commands (can be used multiple times))
3. MAINTAINER - Specify the author of an image.
4. COPY - Copy files and directories.
5. ADD - Add local or remote files and directories(copies files from the host to the image, downloads zip or tar files from a given link, and extracts them automatically).
6. EXPOSE -Describe which ports your application is listening on.
7. WORKDIR - Sets the working directory and Change working directory.
8. CMD - Specify default commands(specifies the command to run when a container is run from the image (can only be used once)).
9. ENTRYPOINTS -Specify default executable(specifies the command to run when a container is run from the image, but allows additional arguments to be passed in (can only be used once)).
10. ENV - Set environment variables.
11. VOLUME - Create volume mounts.
12. USER - Set user and group ID((root, ec2-user, docker, etc.)).
13. LABEL - Add metadata to an image(data about data).
14. ONBUILD - Specify instructions for when the image is used in a build.
15. SHELL - Set the default shell of an image.
16. ARG - Use build-time variables(defines a variable that is passed to the container while building the image).
...
If not specified, the default working directory is /. In practice, if you aren't building a Dockerfile from scratch (FROM scratch), the WORKDIR may likely be set by the base image you're using.Therefore, to avoid unintended operations in unknown directories, it's best practice to set your WORKDIR explicitly.
...
The available [OPTIONS] are: --from, --chown, --chmod  1.2, --link 1.4, --parents 1.7-labs, --exclude 1.7-labs

The COPY instruction copies new files or directories from `src` and adds them to the filesystem of the image at the path `<dest>`. Files and directories can be copied from the build context, build stage, named context, or an image.You can specify multiple source files or directories with COPY.If you specify multiple source files, either directly or using a wildcard, then the destination must be a directory (must end with a slash /).
The last argument must always be the destination. For example, to copy two files, file1.txt and file2.txt, from the build context to /usr/src/things/ in the build container:
...
`Copying from the build context` - When copying source files from the build context, paths are interpreted as relative to the root of the context.
...
*ADD*:- ADD has two forms. The latter form is required for paths containing whitespace.

ADD [OPTIONS] <src> ... <dest>
ADD [OPTIONS] ["<src>", ... "<dest>"]

The available [OPTIONS] are: --keep-git-dir	1.1, --checksum	1.6, --chown, --chmod	1.2, --link	1.4, --exclude	1.7-labs
...
1. If <src> is a local file or directory, the contents of the directory are copied to the specified destination. See Adding files from the build context.
2. If <src> is a local tar archive, it is decompressed and extracted to the specified destination. See Adding local tar archives.
3. If <src> is a URL, the contents of the URL are downloaded and placed at the specified destination. See Adding files from a URL.
4. If <src> is a Git repository, the repository is cloned to the specified destination. See Adding files from a Git repository.
...
*EXPOSE*:-- The EXPOSE instruction informs Docker that the container listens on the specified network ports at runtime. You can specify whether the port listens on TCP or UDP, and the default is TCP if you don't specify a protocol.
...
Available inside of Dockerfile and application code.Set via ENV in Dockerfile or via --env on docker run.Example:
...
1. Chaining ONBUILD instructions using ONBUILD ONBUILD isn't allowed.
2. The ONBUILD instruction may not trigger FROM or MAINTAINER instructions.
...
*HEALTHCHECK*:- The HEALTHCHECK instruction tells Docker how to test a container to check that it's still working. This can detect cases such as a web server stuck in an infinite loop and unable to handle new connections, even though the server process is still running.
...
2. 1: unhealthy - the container isn't working correctly
3. 2: reserved - don't use this exit code
...
Notes about specifying volumes - Keep the following things in mind about volumes in the Dockerfile.
...
2. Changing the volume from within the Dockerfile: If any build steps change the data within the volume after it has been declared, those changes will be discarded when using the legacy builder. When using Buildkit, the changes will instead be kept.
...
If you don't specify a CMD,the CMD of base image will be executed.With no base image and no CMD, you'd get an error.
...
Press Ctrl + space to get tags for image
