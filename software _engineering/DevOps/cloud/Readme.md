...
**uses of cloud services** - Cloud services can be used for everything—from provisioning servers and storage to data analytics, artificial intelligence, and end-to-end security for every application. Below are a few examples.
...
1. Application hosting for an on-demand infrastructure that hosts internal or SaaS applications
2. Backup and storage capability to store data and build dependable backup solutions
3. Content delivery to distribute content worldwide with high data transfer speeds
4. Hosting static and dynamic websites
5. Enterprise IT to host internal-facing or external-facing IT applications in the AWS Cloud
6. Various scalable database solutions from hosted enterprise database software to non-relational database solutions


## Cloud Service Provider

A Cloud Service Provider is a company:-

1. Provides multiple cloud services
2. The cloud services can be chained together to create cloud architectures.
3. Cloud services are available via Single Unified API e.g AWS API
4. Cloud services utilizes metered billing based n usage i.e per seconds,per hour.
5. Have rich monitoring built in
6. Offers automation via infrastructure as Code(IaC).

NOTE:- If a company offers multiple cloud services under a single UI but does not meet most of these requirements,it would be referred to as **Cloud Platform** e.g Twilio,HashiCorp,Databricks.

TIER-1(Top Tier):- Early to market,wide offerings,strong synergies between services,well recognized in the industry.Includes: AWS, Microsoft Azure,GCP,Alibaba Cloud.
TIER-2(Middle Tier):- Backed by well-known company,slow to innovate and turned to specialization.Includes: IBM Cloud,Oracle Cloud,Huawei Cloud,Tencent Cloud.
TIER-3(Light Tier):- Virtual Private Service(VPS) turned to offer core IaaS offering.Simple,cost effective.Include: Vultr,Digital Ocean,Akamai Connected Cloud(Linode)
TIER-4(Private Tier):- IaaS software deployed to run in an organization's own private data center.Includes: OpenStack(rackspace),Apache CloudStack,Vmware vSphere.

- Gartner Magic Quadrant for Cloud:- *Magic Quadrant(MQ)* is a series of market research published by IT consulting firm Gartener that rely on proprietary qualitative data analysis methods to demonstrate market trends,such as direction.maturity and participants.

A cloud service provider can have hundreds of cloud services that are grouped into various type of services.The common types of services for Iaas could be:-

1. *Compute*-Virtual computer that can run applications,programs or code.
2. *Networking*-Virtual network defining internet connections or network isolation between services or outbound to internet.
...
- Dedicated:- Are physical server wholly utilized by a single customer.You have to guess the capacity.Limited by Host Operating System.Can't vertical scale,you need manual migration.Multiple apps can result in conflicts in resource sharing.
- VMs:- You can run multiple Virtual Machines on one machine.Hypervisor lets you run VMs.Limited by Guest OS.Easy to vertical or Horizontal scale.Easy to export and import images for migration.Multiple apps on Single Virtual Machine can result in conflicts in resource sharing.
- Containers:- Virtual Machines running multiple containers.Docker Deamon lets you run multiple containers.Multiple apps can run side by side without being limited to same OS requirements and will not cause conflicts during resource sharing.
- Functions:- Are managed by VMS running managed containers.Known as *serverless compute*.You upload a piece of code,choose amount of memory and duration.Cold starts is a side-effect of the setup.
...
- `Dedicated Server` - One physical machine dedicated to  single business.Expensive but high security.
- `Virtual Private Server(VPS)` - One physical machine dedicated to single machine.The physical machine is virtualized into sub-machines.Better utilization and isolation of resources.
- `Shared Hosting`  - One physical machine,shared by hundred of business.Cheap, limited functionality and poor isolation.E.g godaddy.
- `Cloud computing` - Multiple physical machines that act as one system.The system is abstracted into multiple cloud services.Flexible,secure,cost-effetive.
