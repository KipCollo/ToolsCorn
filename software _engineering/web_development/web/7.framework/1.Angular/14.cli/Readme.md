...
`@schematics/angular` default collection:- appShell,application,class,component,directive,enum,guard,interceptor,interface,library,module,pipe,service,serviceWorker,webWorker.
...
Some libraries have added schematics which will set defaults:-

- `ng add <library>`:- Adds support for an external library to your project.
...
`Angular Update Guide`:- The official Angular Update Guide <update.angular.io> provides a comprehensive step-by-step guide for updating your Angular application. It’s tailored to your specific needs, depending on your current Angular version and the version you want to upgrade to.
...
Angular 21 brings a modern new look to the `ng version` command output:-

- All packages appear in a single table for simplicity, so the separate Angular section is removed.
- New column is added to package table.It shows package version specified in your workspace package.json.
- Vesrions ar unified in one place.
- A new --json option outputs all version information in JSON grouped by category.

```sh
──(collins㉿kali)-[~/Documents/03_Projects/Personal/Hospital-Management-System/hms-ui]
└─$ ng version
...
```sh
┌──(collins㉿kali)-[~/Documents/03_Projects/Academic/Pharmacy/pharmacy-ui]
└─$ ng version
