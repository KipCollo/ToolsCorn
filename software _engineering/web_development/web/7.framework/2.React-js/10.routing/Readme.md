...
A higher-level router that uses the HTML5 history API (pushState,replaceState and popState) to keep the UI in sync with URL.Wraps your entire application and enables you to define different routes internally without reloading the page.
Should appear once at the root level for your component tree.
...
- **Router component**:- A more generic,low-level interface for routing.BrowserRouter is a specific kind of Router that uses the browser's history API.You typically don't use Router directly unless you're customizing the history object(e.g from server-side rendering or React Native)
...
- **Routes component**:- It is a component from the library(v6 and above) used to define a group of Route components.It replaces the Switch component from previous versions.Key features are:-
   1. Routing Math behaviour - Renders first Route element whose path matches current url.Unlike Switch it requires every Route to be wrapped under Routes.
   2. Route nesting support - Enables nested routing,where child routes can inherit and append to the parent route's path.
   3. Element-based API - Instead of component or render,Routes uses the element prop to renedr a component.
