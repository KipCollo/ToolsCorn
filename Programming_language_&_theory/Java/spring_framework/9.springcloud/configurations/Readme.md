...
The loading of configuration management for a microservice occurs during the bootstrapping phase of the microservice.
...
`Implementation choices`:- You can choose among a large number of battle-tested open source projects to implement a configuration management solution:-
1. Etcd - Open source project written in Go. Used for service discovery and key-value management. Uses the raft (https://raft.github.io/) protocol for its distributed computing model.
2. Eureka - Written by Netflix. Extremely battle-tested.Used for both service discovery and key-value management.
3. Consul - Written by Hashicorp. Similar to Etcd and Eureka in features, but uses a different algorithm for its distributed computing model.
4. ZooKeeper - An Apache project that offers distributed locking capabilities. Often used as a configuration management solution for accessing key-value data.
5. Spring Cloud configuration server - An open source project that offers a general configuration management solution with different back ends. It can integrate with Git, Eureka, and Consul as a back end.
...
The Spring Cloud configuration server is a REST-based application that’s built on top of Spring Boot. It doesn’t come as a standalone server. Instead, you can choose to either embed it in an existing Spring Boot application or start a new Spring Boot project with the server embedded it.
