...
This demo showcases the new `spring-boot-starter-opentelemetry` introduced in Spring Boot 4.0, providing a simplified  way to integrate OpenTelemetry observability into your Spring Boot applications.
Spring Boot 4.0 introduces an official OpenTelemetry starter from the Spring team. Unlike previous approaches that required multiple dependencies and complex configuration, this starter provides:
...
//custom.properties
owner.id=1
owner.found=Found owner with id %s
...
public String findOwner(int ownerId){
  if (ownerId % 2 == 0){
    return String.format(ownerFound);
  } else {
    throw new OwnerNotFoundException(environment.getProperty("owner.not.found"));
  }
}
```
...
*Tag* - Categorizes your controllers. Place it at the class level to name and describe a group of endpoints.

*@Operation*: Describes a specific HTTP endpoint. You can specify the summary, description, and hidden status.

*@ApiResponse*: Documents the expected HTTP status codes and returned data types.

*@Parameter*: Explains request inputs, such as path variables, header fields, or query parameters.

*@Schema*: Defines model properties, validation rules, default values, and example data
...
public class UserController {
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Get user by ID", description = "Fetches complete profile details of a single user.")
    @ApiResponse(responseCode = "200", description = "User found", 
                 content = @Content(schema = @Schema(implementation = User.class)))
    @ApiResponse(responseCode = "404", description = "User not found", content = @Content)
    public User getUserById(
        @Parameter(description = "Unique identifier of the user", example = "42")
