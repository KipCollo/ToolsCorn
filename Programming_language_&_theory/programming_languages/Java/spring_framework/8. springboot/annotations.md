...
2. @Qualifier - Specifies the bean to inject when multiple candidates are available to avoid conflicts between the multiple beans, used with @Autowired.
...
1. @EnableConfigurationProperties - Enables support for @ConfigurationProperties beans.
2. @ConfigurationProperties - Binds external config(e.g application.yml) to a POJO.
3. @Profile - Specifies that a bean is only active for a given profile.

## Core SpringBoot Annotations

1. @SpringBootAppication - Combines @Configuration,@EnableAutoConfiguration and @ComponentScan.Entry point for a Spring Boot app.
2. @EnableAutoConfiguration - Automatically configures spring app based on dependencies.
3. @ComponentScan - Scans the package for components configurations, configurations and services.
4. @Configuration - Marks a class a source of bean definitions.
5. @Bean - Declares a bean(an object managed by the Spring container).
