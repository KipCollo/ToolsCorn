...
NOTE:- Map doesn't implements the Collection interface.It is considered a part of the Collection framework even though it isn't technically a Collection.It is a collection
(note the lowercase), though, in that it contains a group of objects. The reason maps are treated differently is that they need different methods due to being key/value pairs.
